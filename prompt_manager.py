...
class PromptManager:
    ...
    def load_prompt_sections(self):
        ...
        按照以下json格式输出：
            ...
            "commands": [
                ...
                {"command": "set_var", "value": {'修炼太玄经': 'True'}}
            ]
        }
        说明：
        ...
        2. 字段详解：
           ...
           - commands: 在本次剧情中（而非本轮剧情的选项中）操作游戏数据的指令列表，每个指令为一个对象。如无指令，则不加该字段。
                ...
                * value: 指令值，格式根据command而定。
                    - set_var: 添加或设定变量值，值为对象{变量名: 变量值},变量值必须是字符串。
                    - del_var: 删除变量，值为变量名。
        ...
        7. 任务、修习武功、技能等也可以当作物品处理。物品名尽量不多于8个字。
        ...
        4. 角色名字使用[]包裹，特定称谓使用<>包裹.
        5. 若要修改物品，应先移除物品，再添加修改后的物品。
        6. 任务、修习武功、技能等也可以当作物品处理。物品名尽量不多于8个字。
    ...
    def get_continuation_prompt(self,
        ...
        经常使用使用变量存储和更新需要持久化存储的精确信息数据、长时间需要的数字等细节数据(例如某NPC的属性、好感等）、金钱等
        修炼的武功、知识、关键情报、任务、技能等，都可以视为物品进行操作，应该经常这样操作。但寻常事件不要记录成物品。
        经常移除无用的旧变量、旧物品。
        所有剧情、好感、关系推进均不要过快。
    ...
    def get_action_mode_prompt(self, player_name: str, cur_desc: str, short_history_desc: str, player_move: str, inventory_text: str = "", attribute_text: str = "", situation_text: Optional[str] = "", custom_prompt: str = "", vars_text: str = ""):
        ...
        * base_probability: 仅type为'check'时有，表示基本成功率，从-1到1的浮点数（0.02表示2%）。
        ...
        2.normal适用于一般事件，check适用于需要分支、判断成功失败的事件(如获取奖励、规避风险、触发事件)；must适用于需要一定门槛才能达成的事件（如获取特殊物品、触发特殊剧情）；
        没有风险或者损失的，也没有大奖励的事件，一律使用normal类型.
        保持normal：check：must的比例约5：3：2。
        3.鼓励使用极端（负数也算）难度值和基础概率值，尽量避免玩家一步登天或者利用巨大捷径。困难或者不合理事件的base_probability可以小于-0.1，甚至小于-0.5。
