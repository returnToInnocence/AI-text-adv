...
class PromptManager:
    ...
    def load_prompt_sections(self):
        ...
        按照以下json格式输出：
                {
            "description": "场景描述",
            "summary": "总结场景为10-25字摘要。含NPC介绍(如有)",
            "options": [
        ...
        2. 字段详解：
           ...
           - commands: 操作游戏持久化数据的指令列表，每个指令为一个对象。如无指令，则不加该字段。
        ...
        3. 规则与注意事项：
           ...
           - 属性改变：当主角成长、学习武功、新知识、获得机缘、中难检定成功，都可能增加属性；而若收到较大伤害、中毒或者其他惩罚场合下，可能损失属性。
        ...
        总结必须简明扼要。有前后冲突的，以后为准覆盖。不重要的信息，若没有叙述或者压缩空间，可以丢弃。但重要信息尽量保留。
        直接输出总结，不要带有任何前缀或者后缀。
        """
